...
public class PermissionUtils {
	...
	public boolean canIgnoreBatterySaving() {
		if (sdk >= 23) {
			return ((PowerManager) ct.getSystemService(Context.POWER_SERVICE))
					.isIgnoringBatteryOptimizations(ct.getPackageName());
		}
		return true;
	}
	...
	public void requestInstallPermission(int yourRequestCode) {
		...
		if (sdk >= 23 && sdk < 29) {
			...
			if (!ct.getPackageManager().canRequestPackageInstalls()) {
				ct.startActivityForResult(new Intent(Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES)
						.setData(Uri.parse(String.format("package:%s", ct.getPackageName()))), yourRequestCode);
			} else {
			}
	...
	public boolean checkPictureInPicturePermission() {
		...
		if (sdk >= 29) {
			return (((AppOpsManager) ct.getSystemService(Context.APP_OPS_SERVICE)).unsafeCheckOpNoThrow(
					AppOpsManager.OPSTR_PICTURE_IN_PICTURE, Process.myPid(),
					ct.getPackageName()) == AppOpsManager.MODE_ALLOWED);
		}

		return (((AppOpsManager) ct.getSystemService(Context.APP_OPS_SERVICE)).checkOpNoThrow(
				AppOpsManager.OPSTR_PICTURE_IN_PICTURE, Process.myPid(),
				ct.getPackageName()) == AppOpsManager.MODE_ALLOWED);
